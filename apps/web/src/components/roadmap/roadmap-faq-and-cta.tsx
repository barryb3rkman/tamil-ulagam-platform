...
export function RoadmapFaq() {
  return (
    <Section tone="ivory" aria-labelledby="roadmap-faq-title">
      <Container
        size="wide"
        className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20"
      >
        <SectionHeading
          id="roadmap-faq-title"
          eyebrow="FREQUENTLY ASKED QUESTIONS"
          title="Clear answers for a roadmap still being built responsibly."
        />
        <dl className="border-global-navy/12 divide-global-navy/12 border-y">
          {roadmapPageContent.faqs.map((faq) => (
...
export function RoadmapFinalCta() {
  const { finalCallToAction } = roadmapPageContent;

  return (
    <Section
      tone="navy"
      spacing="generous"
      aria-labelledby="roadmap-final-cta-title"
