...
export function GetStartedChoices({
  ...
  return (
    <div className="grid gap-6" data-motion-reveal="">
      <div>
        <h2 className="text-global-navy text-2xl font-bold">{heading}</h2>
        <p className="text-slate mt-2 max-w-md leading-7">{description}</p>
      </div>
      ...
      <ul className="motion-pop-group grid gap-3">
        {startableJourneys.map((journey, index) => (
          <li key={journey.id}>
            <Link
              href={journey.href}
              className="group border-global-navy/12 hover:border-heritage-gold/60 focus-visible:ring-focus rounded-card motion-lift relative flex items-start gap-4 overflow-hidden border bg-white p-4 focus-visible:outline-none sm:p-5"
            >
              <span
                aria-hidden="true"
              ...
              <span className="min-w-0 flex-1">
                <span className="text-slate group-hover:text-heritage-maroon block text-[0.66rem] font-bold tracking-[0.18em] uppercase transition-colors">
                  {journey.eyebrow}
                </span>
                <span className="text-global-navy mt-1 block text-lg font-bold">
                  {journey.title}
                </span>
                <span className="text-slate mt-1 block text-sm leading-6">
                  {journey.description}
                </span>
              </span>
              <span
