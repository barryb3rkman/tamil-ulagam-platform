...
export function LegalDecisionRequired({
  ...
  return (
    ...
    >
      <p className="text-fg-accent text-eyebrow-sm">{decision.label}</p>
      <h3 className="text-fg mt-3 text-xl font-semibold">{decision.title}</h3>
      <p className="text-fg-muted mt-3 leading-7">{decision.description}</p>
...
export function LegalOperationalTrigger({
  ...
  return (
    <aside
      ...
      className="border-heritage-gold bg-sunken mt-8 border-l-4 px-5 py-6 sm:px-7"
    >
      <p className="text-warning text-eyebrow-sm">{trigger.label}</p>
      <h3 className="text-fg mt-3 text-xl font-semibold">{trigger.title}</h3>
      <p className="text-fg-muted mt-3 leading-7">{trigger.description}</p>
      <ul className="mt-5 grid gap-3 sm:grid-cols-2">
