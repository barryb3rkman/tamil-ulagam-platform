...
export function LegalSection({ section }: LegalSectionProps) {
  ...
  return (
    <section
      ...
      className="border-hairline/12 scroll-mt-28 border-t py-10 first:border-t-0 first:pt-0 sm:py-12"
    >
      <div className="grid gap-4 sm:grid-cols-[3rem_minmax(0,1fr)] sm:gap-6">
        ...
        <div>
          <h2
            id={headingId}
            className="text-fg text-3xl leading-tight font-semibold tracking-[-0.035em] text-balance sm:text-4xl"
          >
            {section.title}
          </h2>
          <div className="text-fg-body/88 mt-6 grid max-w-3xl gap-4 text-[1.03rem] leading-8">
          ...
          {section.items ? (
            <div className="mt-7">
              ...
              <ul className="border-hairline/12 mt-4 grid border-t sm:grid-cols-2">
                {section.items.map((item) => (
                  <li
                    key={item}
                    className="border-hairline/12 flex gap-3 border-b py-3 leading-7 sm:px-4 sm:odd:pl-0 sm:even:border-l"
                  >
                    <span
                      aria-hidden="true"
