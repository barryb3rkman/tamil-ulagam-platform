  ...
  return (
    <aside className="lg:self-start">
      <nav
        ...
        className="border-hairline/14 bg-raised px-5 py-6 lg:sticky lg:top-28 lg:px-6"
      >
        <h2 className="text-fg text-eyebrow">On this page</h2>
        <ol className="border-hairline/12 mt-5 max-h-[28rem] overflow-y-auto overscroll-contain border-t pr-2 sm:grid sm:max-h-[32rem] sm:grid-cols-2 sm:gap-x-6 lg:block lg:max-h-[calc(100vh-10rem)] lg:pr-2">
          {sections.map((section) => (
            <li key={section.id} className="border-hairline/12 border-b">
              <a
                href={`#${section.id}`}
...
export function RelatedLegalDocuments({ links }: RelatedLegalDocumentsProps) {
  return (
    <nav aria-label="Related legal documents" className="mt-8">
      <h3 className="text-fg text-eyebrow">Related documents and guidance</h3>
      <ul className="mt-4 flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:gap-5">
        {links.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="focus-visible:ring-focus text-fg decoration-heritage-gold hover:text-interactive-blue rounded-sm font-semibold underline underline-offset-4 transition-colors focus-visible:outline-none"
            >
              {link.label}
            </Link>
          </li>
