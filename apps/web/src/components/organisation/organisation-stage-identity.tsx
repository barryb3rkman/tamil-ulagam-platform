    ...
    <div className="surface-card grid gap-7 p-5 sm:p-7 lg:p-8">
      ...
      <div className="grid gap-5">
        <fieldset className="grid gap-3">
          <legend className="text-fg mb-1 text-sm font-semibold">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {organisationCategories.map((option) => {
              ...
              return (
                <label
                  key={option.value}
                  className={`motion-card focus-within:ring-focus rounded-card relative min-h-28 cursor-pointer overflow-hidden border p-4 ${selected ? "border-heritage-maroon bg-heritage-maroon/5 shadow-card" : "border-hairline/12 bg-raised"}`}
                >
                  <input
                    type="radio"
                  ...
                  <span className="flex h-full flex-col justify-between">
                    ...
                    <span className="text-fg-muted mt-2 block text-xs leading-5">
                      {option.description}
                    </span>
                  </span>
                  <span
                    aria-hidden="true"
                    className={`absolute top-3 right-3 grid size-6 place-items-center rounded-full border text-xs font-bold ${selected ? "border-heritage-maroon bg-heritage-maroon text-white" : "border-hairline/15 text-transparent"}`}
                  >
                    ✓
                  </span>
                </label>
      ...
      <div className="border-hairline/10 grid gap-5 border-t pt-6">
        <h3 className="text-fg text-base font-bold">Organisation profile</h3>
        <TextField
